...
const TetrisGame = () => {
  const [tetris, updatePosition, resetTetris, rotateTetris, nextTetris]: UseTetris = useTetris()
  const [board, setBoard, rowsCleared]: UseBoard = useBoard(tetris, resetTetris)
  const [score, setScore, rows, setRows, level, setLevel]: UseGameStats = useGameStats(rowsCleared)
        ...
        <div className='mt-4 grid grid-cols-2 gap-2'>
          {gameOver === null ? (
            <>
              ...
              <button
                onClick={startGame}
                className='cursor-pointer rounded-lg border border-gray-700 bg-white text-xs sm:text-xl p-2 font-bold outline-none transition duration-200 hover:bg-gray-700 hover:text-white sm:col-span-2'
              >
                START GAME
              </button>
            </>
